<template>
  <div name="list" class='app-list'>
    <el-row class='list-box' v-loading='loading' element-loading-spinner="el-icon-loading" element-loading-text="loading....">
      <el-col :span='24'>
        <div>
          <el-steps class="steps-common" finish-status="success" :active='active-1' simple>
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="160px" class="demo-ruleForm" size='small'>
          ...
          <div class="step-three" v-if="active == 1">
            ...
            <el-form-item :label="$t('project_name')" prop="name">
              <el-col :span='23'><el-input size="small" v-model="ruleForm.name"></el-input></el-col>
...
<script>
  import { builderPrjCreate, builderReps, getModelList } from '@/api'
      ...
      submitForm (formName) {
        thisVue.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            builderPrjCreate(params).then(dat => {
              let res = dat.data
              thisVue.loading = false
              if (res && res.errorMessage) {
                thisVue.$message({type: 'error', message: res.errorMessage})
              } else {
                thisVue.$message({type: 'success', message: thisVue.$t('create_project_success')})
...
<style scoped>
  .list-box{background-color: #fff; border:0;min-height: 600px;}
