<template>
  <div>
    <el-upload class="upload-demo" :auto-upload='false' drag action="https://jsonplaceholder.typicode.com/posts/" :on-change='uploadFile' v-loading='loading' element-loading-text="loading..." element-loading-spinner="el-icon-loading">
      <i class="el-icon-upload"></i>
      <div class="el-upload__text"><em>{{ $t('click_upload') }}</em></div>
    </el-upload>
  ...
  export default {
    ...
    methods: {
      uploadFile (file, fileList) {
        if (thisVue.$store.state.env === 'private') {
          thisVue.privateUpload(file.raw)
        } else {
      ...
      submitFile (res, file) {
        ...
        comUploadFile('post', res.url, fd, function (e) {
          ...
          thisVue.$emit('fileUrl', res.url + res.fields.key)
      ...
      privateUpload (file) {
        ...
        let fd = new FormData()
        fd = file
        let url = `https://s3.edgescale.org/signer?username=${params.username}&objectname=${file.name}&type=${type}`
        comUploadFile('put', url, fd, function (e) {
          thisVue.upload_percent = Math.round((e.loaded / e.total) * 100)
        }).then(dat => {
          thisVue.loading = false
          let res = dat.data
          thisVue.$emit('fileName', file.name)
          thisVue.$emit('fileUrl', res)
        }).catch(_ => {
          thisVue.loading = false
        })
      }
