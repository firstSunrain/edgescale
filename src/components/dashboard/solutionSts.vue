    ...
    <el-row>
      <el-col :span='24' class="group-con">
        ...
        <div class="group-chart" v-if='pieData.rows.length != 0'>
          <el-col :span='24'>
            <h4 style='text-align: center;margin-bottom: 20px;'>{{ $t('dash_solution_chart_tit') }}</h4>
        ...
        <div class='com-table'>
          <el-table :data='tableData' size='small'>
            ...
            <el-table-column :label="$t('status')">
              <template slot-scope='scope'>
                <el-button type='danger' size="mini" v-if="scope.row.status == 'offline'">{{ $t('upper_offline') }}</el-button>
                <el-button type='success' size="mini" v-if="scope.row.status == 'online'">{{ $t('upper_online') }}</el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
<script>
import { solutionDeployStat, solutionDeployDev } from '@/api'
let thisVue = null
...
export default {
  data () {
    this.chartEvents = {
      click: function (e) {
        let curRow = thisVue.pieData.rows[e.dataIndex]
        thisVue.curSolution = curRow.id
        thisVue.getTable()
      }
    }
    return {
  ...
  methods: {
    getTable () {
      ...
      solutionDeployDev({limit, offset, solution_id: thisVue.curSolution}).then(dat => {
        let res = dat.data
    ...
    getStatistics () {
      thisVue.loading = true
      solutionDeployStat().then(dat => {
        let res = dat.data
        thisVue.loading = false
        thisVue.pieData.rows = res.map(item => {
          let params = {name: item.name, count: item.count, id: item.id}
          return params
        })
      }).catch(_ => {
        thisVue.loading = false
