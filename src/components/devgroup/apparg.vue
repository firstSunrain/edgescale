    ...
    <el-form :model='createAppForm' size='small' label-width='170px' ref='createAppForm'>
      ...
      <el-form-item :label="$t('host_arguments')">
        <div class="inline-form" v-for='(value, key) in createAppForm.dynamic_ports'>
          <el-input v-model='value.containerPort' type='number' :placeholder="$t('host_port')"></el-input>
          <el-input v-model='value.hostPort' type='number' :placeholder="$t('container_port')"></el-input>
      ...
      <el-form-item :label="$t('volume_arguments')">
        <div class="inline-form" v-for='(value, key) in createAppForm.dynamic_volumes'>
          <el-input v-model='value.hostPath' :placeholder="$t('host_path')"></el-input>
          <el-input v-model='value.mountPath' :placeholder="$t('mount_path')"></el-input>
  ...
  methods: {
    ...
    saveDockerConfig () {
      appDockerConfig({parameters: thisVue.createAppForm}).then(dat => {
        let res = dat.data
        if (res.status === 'fail') {
          thisVue.$alert(res.message, 'Error', {type: 'error'})
        } else {
          thisVue.$message({type: 'success', message: res.message})
          thisVue.$emit('dockerConfig', JSON.stringify(thisVue.createAppForm), thisVue.appVersion)
        }
      }).catch(_ => {
        thisVue.loadingDev = false
      })
