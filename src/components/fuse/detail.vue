    ...
    <el-row class='list-box' v-loading='loading' element-loading-spinner="el-icon-loading" element-loading-text="loading....">
      <el-col :span='24' class='opr-box'>
        ...
        <el-col :span='6' style='text-align:right;'>
          <el-button size='small' class='common-btn' @click='getInfo(curId)'><i class="icon-rotate-cw3"></i> {{ $t('refresh') }}</el-button>
        </el-col>
      </el-col>
      ...
      <el-col :span='24' class='basic-box'>
        <el-collapse v-model='activeNames'>
          ...
          <el-collapse-item name='1'>
            ...
            <ul>
              <li v-for="(value, key) in basicInfo" :key='key'>
                <label style='font-size: 15px;' v-if="key != 'config'">
          ...
          <el-collapse-item name='2'>
            <template slot='title'>
              <span class='collapse-tit'>{{ $t('config') }}</span>
            </template>
            <showCode :codeContent='basicInfo.config'></showCode>
          </el-collapse-item>
        </el-collapse>
      </el-col>
    </el-row>
