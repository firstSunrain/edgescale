      ...
      <el-col :span='12' class='box-card-container col-bottom' v-for="(value, key) in list" :key='key'>
        <el-card class="box-card" :class="setClass(key%6)">
          <div slot="header" class="clearfix">
            <div class='card-tit'>
              <div class="block-w n-link"><router-link :to="{name: 'zoo_data_detail', params: {id: value.id}}">{{value.name}}</router-link></div>
            </div>
          </div>
          <ul class='card-ul'>
            ...
            <ul class="card-ul-inner">
              <li class='li-bottom li-100'>
                <div class="block-type li-100 con-wrap">{{ $t('DESCRIPTION') }}</div>
          ...
          <div class="card-opr-container">
            <el-dropdown class='card-opr' trigger='click' placement='right'>
              ...
              <el-dropdown-menu slot="dropdown">
                <el-dropdown-item v-if="value.url !== null && value.url !== ''"><a :href='value.url' style="color:#333;"><i class='icon-download4'></i> {{ $t('download') }}</a></el-dropdown-item>
                <el-dropdown-item v-if="user_type === 'ADMIN' || showDataset === 'my'"><div @click="delCustomer(value.id)"><i class='icon-trash'></i> {{ $t('delete') }}</div></el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </div>
        </el-card>
...
let thisVue = null
export default {
  props: ['list', 'user_type', 'showDataset'],
  data () {
    return {
    }
  },
