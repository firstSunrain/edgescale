<template>
  <div class="customer-box" v-loading='loading' element-loading-spinner="el-icon-loading" element-loading-text="loading...">
    <div class='com-tit'>Distro List</div>
    <el-row>
        ...
        <el-table :data='tableData' border size='small' v-if="page_list">
          <el-table-column label='Name' width="260px">
            <template slot-scope='scope'>
              <router-link :to="{name: 'zoo_distro_detail', params: {id: scope.row.id}}">{{scope.row.name}}</router-link>
            </template>
          </el-table-column>
          ...
          <el-table-column label='Action' width='100'>
            <template slot-scope='scope'>
              ...
              <el-button class='table-btn btn-red' type='text' icon='icon-trash' @click="deldistro(scope.row.id)"></el-button>
            </template>
          </el-table-column>
        </el-table>
        <modelBlock :list='tableData' @showDialog='showDialog' @deldistro='deldistro' v-else></modelBlock>
      </el-col>
    ...
    <el-dialog :title='dialogTit' :visible.sync='dialogVisible' :show-close='false' :close-on-click-modal='false'>
      <div v-loading='dialogLoading' element-loading-spinner="el-icon-loading" element-loading-text="loading...">
        <el-form ref='form' size='small' :model='form' :rules='rules' label-width='130px'>
          ...
          <el-form-item label='Distro Name' prop='name'>
...
export default {
  ...
  methods: {
    getTable () {
      ...
      eiqList('get', 'distro', thisVue.params).then(json => {
    ...
    deldistro (id) {
      thisVue.$confirm('Are you sure to delete this customer?', 'Delete distro', {type: 'warning'}).then(_ => {
        thisVue.params.id = id
        eiqDetail('delete', 'distro', thisVue.params).then(json => {
          thisVue.$message({
            showClose: true,
    ...
    submitForm (form) {
      ...
      thisVue.$refs[form].validate(valid => {
        if (valid) {
          eiqDetail(method, 'distro', thisVue.params).then(json => {
            thisVue.params.id = ''
            thisVue.params.name = ''
    ...
    closeDialog (form) {
      ...
      thisVue.dialogTit = 'Create distro'
  ...
  mounted () {
    ...
    this.$nextTick(function () {
      let pageHeaderData = {
        'homeRouterName': 'home',
        'firstTitle': 'distro',
        'firstRouterName': 'zoo_distro_list',
        'secondTitle': 'List'
      }
