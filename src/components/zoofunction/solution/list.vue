<template>
  <div class="customer-box" v-loading='loading' element-loading-spinner="el-icon-loading" element-loading-text="loading...">
    <div class='com-tit'>Solution List</div>
    <el-row>
        ...
        <el-table :data='tableData' border size='small' v-if="page_list">
          <el-table-column label='Name' width="260px">
            <template slot-scope='scope'>
              <router-link :to="{name: 'zoo_solution_detail', params: {id: scope.row.id}}">{{scope.row.name}}</router-link>
              <img :src="scope.row.icon_url" width='30' height="30" alt="" class='table-img'>
            </template>
          </el-table-column>
          ...
          <el-table-column label='Action' width='100'>
            <template slot-scope='scope'>
              ...
              <el-button class='table-btn btn-red' type='text' icon='icon-trash' @click="delSolution(scope.row.id)"></el-button>
            </template>
          </el-table-column>
        </el-table>
        <modelBlock :list='tableData' @showDialog='showDialog' @delSolution='delSolution' v-else></modelBlock>
      </el-col>
    ...
    <el-dialog :title='dialogTit' :visible.sync='dialogVisible' :show-close='false' :close-on-click-modal='false'>
      <div v-loading='dialogLoading' element-loading-spinner="el-icon-loading" element-loading-text="loading...">
        <el-form ref='form' size='small' :model='form' :rules='rules' label-width='130px'>
          ...
          <el-form-item label='Solution Name' prop='name'>
          ...
          <el-form-item label='Upload Icon' prop='icon_url'>
            <uploadImg :uploadParams="{type: 'eiq', username}" :data="form.icon_url" ref="upload" @uploadImg='getUploadImg' @imgUrl='getImgUrl'></uploadImg>
          </el-form-item>
        </el-form>
    ...
    <uploadImg v-show="block" ref="upload" @uploadImg='getUploadImg' @imgUrl='getImgUrl'></uploadImg>
  </div>
</template>
...
// import uploadImg from '../minioUpload'
...
export default {
  ...
  methods: {
    ...
    getUploadImg (data) {
      thisVue.uploadImgFile = data
    },
    getImgUrl (data) {
      console.log(data)
      thisVue.form.icon_url = data
    },
    ...
    delSolution (id) {
      thisVue.$confirm('Are you sure to delete this customer?', 'Delete Solution', {type: 'warning'}).then(_ => {
        thisVue.params.id = id
        eiqDetail('delete', 'solution', thisVue.params).then(json => {
          thisVue.$message({
            showClose: true,
    ...
    submitForm (form) {
      ...
      thisVue.params.icon_url = thisVue.form.icon_url
      thisVue.$refs[form].validate(valid => {
        if (valid) {
          eiqDetail(method, 'solution', thisVue.params).then(json => {
    ...
    showDialog (type, val) {
      ...
      if (type === 'edit') {
        thisVue.dialogTit = 'Edit Solution'
        let {name, description, id, icon_url} = val
        thisVue.form = {name, description, id, icon_url}
      }
      // console.log(thisVue.form)
      thisVue.$refs.upload.imageUrl = thisVue.form.icon_url
    },
    closeDialog (form) {
      thisVue.dialogVisible = false
      thisVue.form = {
      ...
      thisVue.dialogTit = 'Create Solution'
  ...
  mounted () {
    ...
    this.$nextTick(function () {
      let pageHeaderData = {
        'homeRouterName': 'home',
        'firstTitle': 'Solution',
        'firstRouterName': 'zoo_solution_list',
        'secondTitle': 'List'
      }
